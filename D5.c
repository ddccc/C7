// c:/bsd/rigel/sort/CC/D5.c
// Date: Sun Jan 05 16:33:09 2020
// (C) OntoOO/ Dennis de Champeaux

// d5 is a quicksort to be applied only on small arrays; the generated partitions
...
static void d5c(void **, void **, int, int, int, int (*)(const void*, const void*));

void d5(void **A, void **C, int N, int M, int (*compare)(const void*, const void*)) {
  //  printf("d5 N %i M %i L %i\n", N, M, M-N);
  int L = M - N;
  if ( L <= 0 ) return;
  int depthLimit = 2.9 * floor(log(L));
  d5c(A, C, N, M, depthLimit, compare);
} // end d3

void d5c(void **A, void **C, int N, int M, int depthLimit, 
  ...
  while ( N < M ) {
    ...
    if ( L <= block ) { // use merge sort
      c2c(A, C, N, M, compare);
      return;
    }
    if ( depthLimit <= 0 ) { // use heapsort
      heapc(A, N, M, compare);
      return;
    }
    depthLimit--;
  ...
  GapClosed: ; // j+1 = i
    ...
    */
    ...
    iswap(N, j, A);
    j--;
    if ( N < j ) d5c(A, C, N, j, depthLimit, compare);
    N = i;
  } // end while ( N < M ) 
}  // end d5c
